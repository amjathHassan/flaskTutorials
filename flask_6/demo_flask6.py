...
def homepage():
    ...
    if form.validate_on_submit():
        ...
        flash(f"Pet : {session['pet']}")
        flash(f"Vaccinated : {session['vaccinated']}")
        flash(f"Gender : {session['gender']}")
        flash(f"Food Choice : {session['food_choice']}")
        flash(f"FeedBack : {session['Feedback']}")
